...
Mat FingerCount::findFingersCount(Mat input_image, Mat frame) {
	...
	static uint8_t fingers[6];
	uint8_t max_count = 0;
	static uint8_t No_of_fingers;
	Option OptionToSend;
	...
	putText(frame, to_string(No_of_fingers),center_bounding_rect, FONT_HERSHEY_PLAIN, 10, color_red);
	putText(contours_image, to_string(No_of_fingers), center_bounding_rect, FONT_HERSHEY_TRIPLEX, 10, color_purple);
	counter++;
	fingers[(int)(filtered_finger_points.size())]++;
	if(counter > NO_OF_SAMPLES){
		for(uint8_t i = 0; i<6; i++ ){
			if(max_count < fingers[i]){
				No_of_fingers = i;
				max_count = fingers[i];
			}
		}
		if(LookingforOption && (No_of_fingers != 0) && start_sending)
		{
			OptionToSend.option = (option_e)No_of_fingers;
			#ifdef LOGGING
			LOG(INFO, "Hand-Detection Task : Sending Option %d\n",No_of_fingers);
			#endif
			SEND_OPTION(QueueOption,OptionToSend);
		}
